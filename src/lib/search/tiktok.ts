...
function normalizeTikTokResult(r: Record<string, unknown>, idx: number): SearchResult {
...
export async function runTikTokSearch(
  ...
  onStatus('Starting TikTok search…')
  ...
  onStatus('TikTok search: RUNNING…')
    ...
    const poll = async () => {
      try {
        ...
        if (newItems.length > 0) {
          ...
          if (organic.length > 0) {
            onNewResults(organic.map((r, i) => normalizeTikTokResult(r, idxOffset + i)))
            idxOffset += organic.length
          }
        }

        const status = await getRunStatus(run.id, token)
        onStatus(`TikTok search: ${status}…`)
        ...
        if (['SUCCEEDED', 'FAILED', 'ABORTED', 'TIMED-OUT'].includes(status)) {
          ...
          if (finalItems.length > 0) {
            ...
            organic = organic.filter(filterTt)
            if (organic.length > 0) onNewResults(organic.map((r, i) => normalizeTikTokResult(r, idxOffset + i)))
          }
          resolve(); return
        }
